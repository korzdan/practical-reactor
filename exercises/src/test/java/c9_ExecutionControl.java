 ...
 * helps with that by providing many mechanisms to execute work in parallel.
 *
 * Read first:
 *
...
public class c9_ExecutionControl extends ExecutionControlBase {
   ...
   * multiple notifications, for better UX you want to slow down appearance between notifications by 1 second. Pay
   * attention to threading, compare what code prints out before and after solution. Explain why?
   */
  @Test
  ...
  /**
   * You are using free access to remote hosting machine. You want to execute 3 tasks on this machine, but machine will
   * allow you to execute one task at a time on a given schedule which is orchestrated by the semaphore. If you
   * disrespect schedule, your access will be blocked. Delay execution of tasks until semaphore signals you that you can
   * execute the task.
   */
  @Test
  public void ready_set_go() {
    ...
    Flux<String> tasks = tasks()
        .concatMap(task ->
            task.delayUntil(v -> semaphore().next())
        );
  ...
  /**
   * Make task run on thread suited for short, non-blocking, parallelized work. Answer: - Which types of schedulers
   * Reactor provides? - What is their purpose? - What is their difference?
   */
  @Test
  ...
  /**
   * Make task run on thread suited for long, blocking, parallelized work. Answer: - What BlockHound for?
   */
  @Test
  ...
  /**
   * Adapt the code so tasks are executed in parallel, but task results should preserve order in which they are
   * invoked.
   */
  @Test
   ...
   * metadata, printing out metadata, and mapping to json can be done in parallel. Then branch in before appending
   * events to store. `appendToStore` must be invoked sequentially!
   */
  @Test
